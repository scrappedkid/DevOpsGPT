...
def chatCompletion(context, fackData="", bill: bool = True):
    ...
    try:
        ...
        print(f"chatCompletion failed 1 time:{str(e)}")
        try:
            message, total_tokens, success = obj.chatCompletion(context, fackData, bill)
        except Exception as e:
            print(f"chatCompletion failed 2 time:{str(e)}")
            traceback.print_exc()
            try:
                message, total_tokens, success = obj.chatCompletion(context, fackData, bill)
            except Exception as e:
                print(f"chatCompletion failed 3 time:{str(e)}")
                traceback.print_exc()
                try:
                    message, total_tokens, success = obj.chatCompletion(context, fackData, bill)
                except Exception as e:
                    print(f"chatCompletion failed 4 time:{str(e)}")
                    traceback.print_exc()
                    try:
                        message, total_tokens, success = obj.chatCompletion(context, fackData, bill)
                    except Exception as e:
                        print(f"chatCompletion failed 5 time:{str(e)}")
                        traceback.print_exc()
                        try:
                            message, total_tokens, success = obj.chatCompletion(context, fackData, bill)
                        except Exception as e:
                            print(f"chatCompletion failed 6 time:{str(e)}")
                            traceback.print_exc()
